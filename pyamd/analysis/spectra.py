...
from pyamd.utilities import root6, physics, helper
import pathlib
import os
import pandas as pd
import numpy as np
...
class PtRapidityCMS:
    # default directory in AMD
    DIR = f'{PROJECT_DIR}/isoscaling_analysis/result/spectra2d'

    def __init__(self, particle, df=None, path=None, reaction='Ca48Ni64E140', skyrme='SkM', impact_parameter=(0., 3.), uball_multiplicity=(1, 25), mode='all'):
        ...
        if df is None:
            if path is None:
                path = f'{self.DIR}/{reaction}_{skyrme}_bmin{impact_parameter[0]:.1f}_bmax{impact_parameter[1]:.1f}_Ncmin{uball_multiplicity[0]}_Ncmax{uball_multiplicity[1]}.root'
            spectra_file = SpectraFile(path)
            df = spectra_file.get_histogram(
                particle=particle, keyword=f'h2_pta_rapidity_cms_{mode}')
    ...
    def Multiplicity(self):
        # multiplicty per event = integral of the normalized spectra
        # only works for 'all'
        # primary events are taken with weight = passed_decays / ndeacys
        # e.g. table21 -> table3 10 events, x of them passed -> w = x/10
        return np.sum(self.df['z'])

    def PtRapidityLAB(self, range=[[0, 1], [0, 600]], bins=[100, 600]):
        df = self.df.copy()
        dx = np.diff(np.unique(df.x)[0:2])
        x = df.x + dx * np.random.uniform(-0.5, 0.5, size=len(df.x))
        x = x + 0.5 * np.log((1+self.betacms)/(1-self.betacms))
        x /= self.beam_rapidity
        df.x = x

        hist, xedges, yedges = np.histogram2d(
            df.x, df.y, weights=df['z'], range=range, bins=bins)
        histerr, xedges, yedges = np.histogram2d(
            df.x, df.y, weights=df['z_err']**2, range=range, bins=bins)
        ...
        df['z_ferr'] = np.where((df['z'] > 0.0), df['z_err']/df['z'], 0.0)
        return df
...
class PtRapidityLAB:
    ...
    def __init__(self, particle, df=None, path=None, reaction='Ca48Ni64E140', skyrme='SkM', impact_parameter=(0., 3.), uball_multiplicity=(1, 25), mode='all'):
        ...
        if df is None:
            ...
            spectra_file = SpectraFile(path)
            df = spectra_file.get_histogram(
                particle=particle, keyword=f'h2_pta_rapidity_lab_{mode}_{particle}')
            df = hist_reader.hist2d_to_df(df, keep_zeros=False)
...
if __name__ == '__main__':
    ...
    spectra = PtRapidityLAB(particle='p', reaction='Ca48Ni64E140', skyrme='SkM', impact_parameter=(
        0., 3.), uball_multiplicity=(10, 25), mode='all')
    # df = spectra.PtRapidityLAB()
    # spectra = PtRapidityLAB(reaction='Ca48Ni64E140', particle='p', df=df)

    fig, ax = plt.subplots()
    spectra.plotPtRapidity(ax, norm=LogNorm())
    # # spectra.plotPtSpectrum(ax)
    # # spectra.plotRapidityCMS(ax)
    fig.savefig('test.png')

    # df = spectra.EkinThetaCMS(correct_coverage=True)
    # ekin = EkinThetaCMS('p', df)
    # fig, ax = plt.subplots()
    # ekin.plotEkinThetacMS(ax, norm=LogNorm())
